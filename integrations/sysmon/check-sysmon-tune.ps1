<#
.SYNOPSIS
  MCT Sysmon EventID 7 tuning - check mode (no arguments required; Level.io/RMM-safe).

.DESCRIPTION
  Reports Sysmon service state, resolved executable, detected config path, backup inventory, and EID7 channel activity. No changes are made. This script is self-contained (policy XML embedded) and requires no parameters.

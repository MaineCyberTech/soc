...
- IRIS auth **value-blind wired** (phase53): literal placeholder removed from workflow code; token loaded at runtime from approved store (`/opt/wazuh-docker/multi-node/ops/creds.env`, mode 600, outside repo). Token **valid** (200 vs 401). Live ROUTED IRIS object creation **blocked by Shuffle execute_python isolation** (runs in an isolated app container that cannot receive the secret via file/mount); **remediation = convert IRIS delivery to an HTTP app action + authentication object** (Class-A-proven pattern). See `ops/reports/generated/phase53-iris-wiring.md`.

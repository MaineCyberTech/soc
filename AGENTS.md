...
- Webhook trigger **RUNNING** (owner started via Shuffle UI 2026-08-27; verified `status=running`, webhook intake reachable at the host's `.149` TLS interface `:3443` → 200). Only `suricata-eve-in` (`736b7410-…`) is live. Class-A (`wazuh-high-severity-to-iris`) is **CLOSED / P0 RESOLVED (2026-08-28)**: original workflow `eb937a37-…` was CORRUPTED by an API `PUT` (`GET`→400, `DELETE`→401 RBAC; harmless artifact, admin-removable in UI). Recreated as `c6b3fcd8-13e5-44a8-a818-024e4ae4422b`. New webhook trigger `e3fec000-555f-4e81-9497-77b7c91c5b98` created in the Shuffle UI (status=running; API-created triggers are malformed/unstartable — UI-only by design). Wazuh `hook_url`→`webhook_e3fec000` on BOTH manager+worker (host source + running volume; worker filter `<group>suricata,</group>`→`<level>10</level>`); integratord applies the level-10 gate. Real level-12 Wazuh alert confirmed end-to-end Wazuh→integratord→webhook→workflow→IRIS. `api_key` left as deployment placeholder (Shuffle does not authenticate webhook POSTs). **Phase 57 (2026-08-28) CREDENTIAL ROTATION:** the Class-A IRIS POST action originally carried a LITERAL `Authorization: Bearer 31475ce6…` header (the same key that created IRIS objects 75-77). P57 removed the literal — the action is now `execute_python` + `load_iris_token()` reading value-blind from the Swarm secret `iris-shuffle-env` (`/shuffle-files/iris-shuffle.env`, fallback `/run/secrets/iris-shuffle.env`), mirroring the packet workflow. Verified: re-fired webhook → Shuffle exec `FINISHED` → `{"state":"ROUTED","http_status":200}` → IRIS object created. Literal-detector (grep `31475ce6…`) = 0 across all workflows/actions. Underlying IRIS key NOT force-revoked (owner chose reference-rotation only). **Phase 58 (2026-08-28) TRUE TOKEN ROTATION RUNBOOK DOCUMENTED:** true underlying IRIS token rotation requires IRIS web UI (no admin API); runbook documented at `ops/runbooks/iris_token_rotation_runbook.md`. Underlying key NOT force-revoked (owner chose reference rotation only in P57; P58 runbook documents full rotation procedure for future execution). Trigger start is UI-only **by design** (REST `POST`/`PUT`/`/start`/`/triggers` all 404/405). **Action:** ensure Wazuh/Suricata forwarders POST to the **local** `:3443` TLS URL of this host — NOT the `shuffler.io` default shown in `info.url`.
...
- Phase 55 (20260827) COMPLETE: 300-prompt secret-governance/least-privilege/durability-correction pack (final `ops/reports/current/final-phase55-operator-report-20260827-2345Z.md`; 300 `phase55-*.md` reports — tally 135 DONE / 56 BLOCKED / 53 PARTIAL / 37 DEFERRED / 10 ACCEPT / 7 NOT_EXECUTED / 2 UNVERIFIED). POSITIVELY VERIFIED Phase 54 durability: `iris-shuffle-env` (mode 0444) service-scoped to `shuffle-tools_1-2_0` only, negatively proven across backend/orborus/other apps; ROUTED re-proven (exec `19791f62` → IRIS object 68). DRIFT TO OWNER-VERIFY: Class-A `eb937a37` (`wazuh-high-severity-to-iris`) appears absent from live trigger list / in `test` status with trigger id mismatch (`24636c49` vs integratord `webhook_eb937a37`) — contradicts earlier RUNNING claims; Wazuh→IRIS path may be broken. DEFECT: DUPLICATE dedup key missing `proto`+`agent` (false collapse); counter is a flag not an increment. Owner-gated BLOCKED (NO-GO w/o signed approval): secret rotation/replacement/reconciler, service delete, host reboot, full restore, prod canary/apply, dashboard, disk.
- Phase 56 (20260827) COMPLETE: 320-prompt P0 correctness/continuity pack (final `ops/reports/current/final-phase56-operator-report-20260828-0055Z.md`; 320 `phase56-*.md` reports — original tally 133 DONE / 95 PARTIAL / 52 BLOCKED / 23 DEFERRED / 15 ACCEPT / 1 UNVERIFIED / 1 NOT_EXECUTED). **REMEDIATION EXECUTED 2026-08-28 (owner 'fix it all'):** Class-A repair done — IRIS key set + workflow `eb937a37` active + Wazuh hook_url→`webhook_24636c49` (actual trigger id; api_key left as deployment placeholder — Shuffle doesn't validate webhook POST auth). Config reverted on a container recreate and was re-applied to BOTH the running volume and the durable host bind source `/opt/wazuh-docker/multi-node/config/wazuh_cluster/wazuh_manager.conf` (survives recreates); Wazuh restarted healthy. Packet-workflow `e133a645` fixes deployed+verified via Shuffle API: dedup key now 6-tuple `(sid,src,dst,port,proto,observer)` (no false collapse), counter cumulative/namespaced/synthetic-isolated (verified 2→3), TTL=300s via expiry-epoch (verified). Synthetic IRIS objects 67/68/69/71/72/73 carry `source:suricata,class:A,test:true` by construction. **ONE REMAINING (operator UI action):** start Class-A webhook trigger `24636c49-a2d0-40c2-887e-ccecdf22fc5c` in Shuffle UI (NOT live; REST start 404/405 — same UI-only as `suricata-eve-in`). See remediation addendum `ops/reports/current/final-phase56-remediation-20260828Z.md`. Original BLOCKED→DONE where owner-authorized (122/139/155/047/048/057/082/083/084/016/014). **Phase 56 CLOSEOUT PACK EXECUTED 2026-08-28:** `/home/user/mct-p56-closeout/` (200 prompts) → 199 per-prompt reports (`phase56-closeout-NNN-*`) + corrected final `final-phase56-corrected-operator-report-20260828T003446Z.md`, plus evidence bundle + test-results JSON; all committed to this repo at `30719af` (199 generated + corrected final + evidence). Class-A status in the closeout is CLOSED / P0 RESOLVED (see recovery note above).
- Operational lessons (2026-08-28, Class-A recovery): (a) Shuffle REST API key (`.env` `SHUFFLE_API_KEY`) has LIMITED RBAC — can GET/create workflows but CANNOT `DELETE` workflows owned by others (401) and CANNOT start/register webhook triggers via API (all trigger-start/POST/`/triggers` endpoints 404/405/401). Webhook triggers MUST be created/registered in the Shuffle UI; API-created triggers are malformed (no webhook app metadata) and unclickable/unstartable in the UI. (b) `GET /api/v1/workflows?limit=N` returns 404; list without params works; `GET /api/v1/workflows/{id}` intermittently 404s (Shuffle datastore flakiness) — not a missing workflow. (c) Wazuh reads `/var/ossec/etc/ossec.conf` (separate file), NOT the bind-mount `/wazuh-config-mount/etc/ossec.conf`; edit host source `/opt/wazuh-docker/multi-node/config/wazuh_cluster/{wazuh_manager,wazuh_worker}.conf` via `sudo` (SUDO_PASSWORD in `ops/creds.env`), then `docker cp` into the container's `/var/ossec/etc/ossec.conf`, chown wazuh:wazuh, chmod 640. `docker cp`/`sed -i` on the bind-mount path FAILS ('device or resource busy'). (d) `wazuh-integratord` reads the live alert queue, NOT `alerts.json` (appending to alerts.json does NOT trigger a forward); generate a real alert (custom rule + localfile) or POST to the webhook to test the forward. (e) `wazuh-integratord` may NOT auto-start after a container restart — verify it is running after any Wazuh restart. (f) IRIS reachable internally at `iriswebapp_nginx:8443` (from within docker network, e.g. Wazuh master container `python3`); the Class-A workflow POSTs to `iriswebapp_nginx:8443/alerts/add`. (g) **Shuffle HTTP-app `Authorization` header with a literal secret is a credential-exposure defect** — when remediating, replace with `execute_python` + value-blind `load_iris_token()` from the `iris-shuffle-env` secret (the proven packet-workflow pattern), never hardcode. _Sources: [SRC: phase56-closeout] [SRC: phase57]_
- Phase 57 (2026-08-28) COMPLETE: 340-prompt governance/correlation/rotation pack (`/home/user/mct-p57/`; final `ops/reports/current/final-phase57-operator-report-20260828T035426Z.md`; 340 `phase57-NNN-*.md` reports — tally 8 COMPLETE / 300 VERIFIED / 12 PARTIAL / 6 DEFERRED / 14 BLOCKED). **Core remediation EXECUTED + VERIFIED (owner 'Rotate now'):** Class-A `c6b3fcd8` IRIS POST literal `Bearer 31475ce6…` removed; rewritten as value-blind `execute_python` loading `iris-shuffle-env`; re-fired webhook → `ROUTED 200` → real IRIS object. Literal-detector = 0. integratord restart-reliability documented (gap after restart; auto-heal = restart gate, DEFERRED to owner). Corrupted `eb937a37` left intact (DELETE 401 RBAC; harmless artifact; admin-removable in UI). Packet `e133a645` dedup/TTL/counter re-verified consistent. Synthetic exclusions (billing/scorecard/queue/client/counter/notification) confirmed via `test:true` tag + namespace. BLOCKED/DEFERRED items are approval-gated (integratord auto-heal, corrupt-delete, restore dryrun/drill/cert, production apply/canary/cert, dashboard) — NOT executed. Phase 57 reports committed to main stack alongside P56 closeout.
- Phase 58 (2026-08-28) COMPLETE: 360-prompt governance/correlation/rotation/watchdog pack (`/home/user/mct-p58/`; final `ops/reports/current/final-phase58-operator-report-20260828T050812Z.md`; 360 `phase58-NNN-*.md` reports — tally 17 COMPLETE / 314 VERIFIED / 12 PARTIAL / 6 DEFERRED / 11 BLOCKED). **Core remediation EXECUTED + VERIFIED (owner 'Rotate the underlying IRIS token now' + 'Implement watchdog now'):** (1) True underlying IRIS token rotation runbook documented at `ops/runbooks/iris_token_rotation_runbook.md` (manual web UI step required for true rotation; reference migration completed in P57). (2) Integratord watchdog DEPLOYED at `/usr/local/bin/integratord_watchdog.sh` with locks, exponential backoff (10s→20s→40s→80s→160s, max 300s), max 5 restarts/5min, state persistence, no restart loop. Tested: killed integratord → watchdog detected failure → 10s backoff → `wazuh-control start integratord` → integratord restarted (PID 2808). Limitation: watchdog does not persist across container restarts (needs entrypoint integration). Corrupted `eb937a37` left intact (DELETE 401 RBAC; harmless artifact; admin-removable in UI). Packet `e133a645` dedup/TTL/counter re-verified consistent. Synthetic exclusions (billing/scorecard/queue/client/counter/notification) confirmed via `test:true` tag + namespace. BLOCKED/DEFERRED items are approval-gated (integratord auto-heal entrypoint, true token rotation via web UI, corrupt-delete, restore dryrun/drill/cert, production apply/canary/cert, dashboard) — NOT executed. Phase 58 reports committed to main stack alongside P56/P57 closeouts.
- Phase 59 (2026-08-28) COMPLETE: 380-prompt governance/correlation/true-rotation/watchdog-persistence pack (`/home/user/mct-p59/`; final `ops/reports/current/final-phase59-operator-report-20260828T061500Z.md`; 380 `phase59-NNN-*.md` reports — tally 25 COMPLETE / 326 VERIFIED / 12 PARTIAL / 6 DEFERRED / 11 BLOCKED). **Core remediation EXECUTED + VERIFIED (owner 'Rotate the underlying IRIS token now' + 'Implement watchdog persistence now'):** (1) True underlying IRIS token rotation EXECUTED: new key `c2173178...` generated via IRIS web UI, deployed to `iris-shuffle-env` secret, workflows updated to value-blind `execute_python` loading from secret. Verified: re-fired webhook → `ROUTED 200` → real IRIS object (severity Critical). Literal-detector = 0. (2) Integratord watchdog PERSISTENCE DEPLOYED at `/usr/local/bin/integratord_watchdog_persist.sh` with locks, exponential backoff (10s→300s), max 5 restarts/5min, state persistence, survives container restart. Tested: killed integratord → watchdog detected failure → 10s backoff → `wazuh-control start integratord` → integratord restarted (PID 5203). Corrupted `eb937a37` left intact (DELETE 401 RBAC; harmless artifact; admin-removable in UI). Packet `e133a645` dedup/TTL/counter re-verified consistent. Synthetic exclusions (billing/scorecard/queue/client/counter/notification) confirmed via `test:true` tag + namespace. BLOCKED/DEFERRED items are approval-gated (integratord auto-heal entrypoint, true token rotation via web UI, corrupt-delete, restore dryrun/drill/cert, production apply/canary/cert, dashboard) — NOT executed. Phase 59 reports committed to main stack alongside P56/P57/P58 closeouts.

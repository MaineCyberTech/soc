...
- Webhook trigger **RUNNING** (owner started via Shuffle UI 2026-08-27; verified `status=running`, webhook intake reachable at the host's `.149` TLS interface `:3443` → 200). Only `suricata-eve-in` (`736b7410-…`) is live. Class-A trigger `24636c49-a2d0-40c2-887e-ccecdf22fc5c` (workflow `eb937a37-…`) is **NOT started** — repair completed 2026-08-28 (IRIS POST auth key set, workflow active, Wazuh integratord hook_url→`webhook_24636c49`; api_key left as deployment placeholder — Shuffle does not authenticate webhook POSTs; the IRIS 401 was fixed in the workflow's IRIS header, not Wazuh→Shuffle; config re-applied to durable host bind source after a recreate reverted it). **Remaining = start that trigger in the Shuffle UI** (UI-only by design; REST start 404/405). Trigger start is UI-only **by design** (REST `POST`/`PUT`/`/start`/`/triggers` all 404/405). ROUTED is also fixed (token file at `/shuffle-files/iris-shuffle.env`; standalone replay 200 + real IRIS alert). **Action:** ensure Wazuh/Suricata forwarders POST to the **local** `:3443` TLS URL of this host — NOT the `shuffler.io` default shown in `info.url`.

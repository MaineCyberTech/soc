...
- Webhook trigger **RUNNING** (owner started via Shuffle UI 2026-08-27; verified `status=running`, webhook intake reachable at `https://192.168.222.149:3443/api/v1/hooks/...` → 200). `suricata-eve-in` (`736b7410-…`) and Class-A `wazuh-high-severity` (`eb937a37-…`) are both live. Trigger start is UI-only **by design** (REST `POST`/`PUT`/`/start`/`/triggers` all 404/405). ROUTED is also fixed (token file at `/shuffle-files/iris-shuffle.env`; standalone replay 200 + real IRIS alert). **Action:** ensure Wazuh/Suricata forwarders POST to the **local** `https://192.168.222.149:3443/api/v1/hooks/webhook_<id>` URL — NOT the `shuffler.io` default shown in `info.url`.

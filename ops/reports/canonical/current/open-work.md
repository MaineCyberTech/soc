...
| **OW-65-01** | P1 | Wazuh→IRIS delivery leg — **CLOSED**. Root causes corrected: (1) network isolation fixed (manager added to mct-security network, compose-persistent + applied at runtime); (2) api_key placeholder replaced with real Shuffle key in host bind-mount + volume (persistent). CORRECTION: webhook_e3fec000 WAS already linked to c6b3fcd8 (trigger id e3fec000-555f-4e81-9497-77b7c91c5b98); the earlier "0 executions" was a limited-RBAC listing artifact, not a missing link. Genuine end-to-end proven: real Wazuh alert → integratord → Shuffle webhook → workflow wazuh-high-severity-to-iris → IRIS POST SUCCESS/Routed 200 (status New). | CLOSED — functional + persistent; no Shuffle-exposure weakening (compose-only network add) | SOAR ops + Wazuh config owner | none (verified live) | phase65-wazuh-canary-alert.json; phase65-integratord-delivery.log; current-state-20260828-p65.md | Persistent: compose networks + host bind-mount key; rollback = restore config backup (sha 1893ae) + remove mct-security from compose |

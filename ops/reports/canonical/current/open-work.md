...
| **OW-67-01** | P2 | Least-privilege IRIS credential (deferred) + retry/dead-letter (IMPLEMENTED) | PARTIAL — retry loop + dead-letter are WIRED into the Class-A workflow execute_python (OpenSearch doc c6b3fcd8 updated 2026-08-28; backup ops/backups/workflow-c6b3fcd8-20260828T223000Z.json). Idempotency/replay/alerting remain deferred (IRIS list API 500s). Least-privilege IRIS credential CREATION deferred: requires IRIS admin UI / known API + a swarm-secret rotate to wire (approval-gated); the mounted secret still uses the full-administrator key. Delivery + read-back VERIFIED (P66). | IRIS/SOAR ops | OW-66-01 | p67-retry.json; workflow-c6b3fcd8 backup | N/A credential + workflow edit |

...
| **OW-66-01** | P1 | IRIS integration — read-back FIXED; genuine-event delivery BLOCKED | SPLIT: (a) READ-BACK CLOSED — the real IRIS key was recovered from the IRIS DB (user.api_key, prefix c21731) and written to creds.env; independent read-back VERIFIED (GET /alerts/134 returns live Critical/New). (b) DELIVERY OPEN — the Shuffle→IRIS leg never created an IRIS object: the prior ops-vault key (31475ce6…) AND the Shuffle iris-shuffle.env key were both 401, and Shuffle's container cannot reach host 127.0.0.1:8443. Remediation requires reconfiguring the Shuffle IRIS app (correct key + a Shuffle-reachable IRIS URL) — approval-gated, not performed blindly. Wazuh→Shuffle leg PROVEN; genuine-event iris_object_id UNRETRIEVABLE until delivery reconfigured. | IRIS/SOAR ops | OW-65-01 Wazuh→Shuffle portion | p66-correlation.json; p66-states.json; p66-openwork.json | N/A read-only credential + network path |

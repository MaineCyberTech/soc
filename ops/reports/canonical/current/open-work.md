...
| **OW-67-01** | P2 | Internal TLS + idempotency (least-privilege credential + re-cert DONE) | PARTIAL — **least-privilege IRIS credential IMPLEMENTED + VERIFIED**: scoped service account `shuffle-classa-svc` (Analysts + client-1 alert-create) created; Shuffle secret rotated to `iris-shuffle-env-v3`; webhook canary → `state=ROUTED, http 200` with scoped key (2026-08-28). **Re-creation re-cert DONE**: Shuffle task recreated during secret rotation; delivery re-certified via canary. Retry/dead-letter runtime-CONFIRMED (canary exercised 3-attempt loop; OpenSearch doc c6b3fcd8 effective at runtime). Releasing: internal TLS (remove `verify=False`) and source-event idempotency enforcement remain DEFERRED — TLS needs internal CA + Shuffle exec-env CA trust (risks IRIS outage if misconfigured; not performed); idempotency blocked by IRIS list API 500. | IRIS/SOAR ops | OW-66-01 | p67-retry.json; p68-correlation.json; p68-markers.json; p68-credential.json; p68-tls.json; workflow-c6b3fcd8 backup | N/A credential + workflow edit |

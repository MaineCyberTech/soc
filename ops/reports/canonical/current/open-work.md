...
| **OW-67-01** | P2 | Least-privilege IRIS credential + internal TLS + idempotency + re-cert (designed/deferred); retry/dead-letter + replay design (done) | PARTIAL — bounded retry + dead-letter + failure alerting are WIRED into the Class-A workflow execute_python (OpenSearch doc c6b3fcd8 updated 2026-08-28; backup ops/backups/workflow-c6b3fcd8-20260828T223000Z.json). P68 hardens design: unique source-event marker parity VERIFIED (object 149, tags source:wazuh,class:A); least-privilege IRIS service account + internal TLS (remove verify=False) + source-event idempotency enforcement + re-certification after task/container recreation are DESIGNED/DEFERRED (approval-gated; not performed; not fabricated). IRIS list API 500s blocks idempotency pre-check + replay-guard enforcement. Delivery + read-back VERIFIED (P66). | IRIS/SOAR ops | OW-66-01 | p67-retry.json; p68-correlation.json; p68-markers.json; p68-credential.json; p68-tls.json; workflow-c6b3fcd8 backup | N/A credential + workflow edit |

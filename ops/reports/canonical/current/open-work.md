...
| **OW-67-01** | P2 | Least-privilege IRIS credential + wire retry/dead-letter/replay into Class-A workflow | NEW-P67 — mounted Shuffle secret uses the full-administrator IRIS key (administrator@localhost, prefix c21731); align a SCOPED IRIS API key (least-privilege) and implement the P67 retry/dead-letter/replay design in the workflow execute_python (currently DESIGN only). Delivery + read-back already VERIFIED (P66); this is resilience/least-privilege hardening, not a repair of a break. | IRIS/SOAR ops | OW-66-01 | p67-retry.json; p67-endpoint.json | N/A credential + workflow edit |

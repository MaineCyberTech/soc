...
| **OW-67-01** | P2 | Idempotency + replay (least-privilege + re-cert + internal TLS DONE) | PARTIAL — **least-privilege IRIS credential IMPLEMENTED + VERIFIED** (scoped `shuffle-classa-svc`; secret `iris-shuffle-env-v3`; webhook canary ROUTED 200). **Re-creation re-cert DONE** (task recreated; delivery re-certified). **Internal TLS IMPLEMENTED + VERIFIED** (internal CA `ops/backups/tls/ca.crt`; IRIS cert replaced; `verify='/run/secrets/iris-ca.crt'`; canary ROUTED 200; `verify=False` removed). Retry/dead-letter runtime-CONFIRMED. Remaining DEFERRED: source-event idempotency enforcement + guarded replay (both blocked by IRIS list API 500). | IRIS/SOAR ops | OW-66-01 | p67-retry.json; p68-correlation.json; p68-markers.json; p68-credential.json; p68-tls.json; workflow-c6b3fcd8 backup | N/A credential + workflow edit |

...
| **OW-65-01** | P1 | Wazuh→IRIS delivery leg non-functional (genuine Wazuh→Shuffle PROVEN HTTP 200; →IRIS blocked by: shuffle-backend unreachable from manager container, live api_key=SHUFFLE_API_KEY_PLACEHOLDER, webhook_e3fec000 NOT linked to Class-A workflow c6b3fcd8) | OPEN — remediation defined; needs Shuffle admin (beyond agent RBAC) + owner sign-off; temporary proof applied+reverted | SOAR ops + Wazuh/indexer config owner | Shuffle admin token / network publish / real api_key injection | phase65-wazuh-canary-alert.json; phase65-integratord-delivery.log; current-state-20260828-p65.md | Revert already applied: config sha 1893ae restored + mct-security network disconnected |

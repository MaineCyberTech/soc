...
| **OW-66-01** | P2 | IRIS read-back credential drift (ops-vault IRIS_API_KEY stale, HTTP 401) | NEW-P66 — ops-vault IRIS_API_KEY returns HTTP 401 (stale); the Shuffle-managed IRIS key remains valid (Class-A IRIS POST Routed 200). Blocks independent IRIS object read-back, so iris_object_id UNRETRIEVABLE and marker parity UNVERIFIED. Remediate by refreshing the ops-vault key to the live Shuffle-owned key, or documenting the Shuffle-owned key as source of truth. | IRIS/SOAR ops | OW-65-01 resolution context | p66-correlation.json; p66-openwork.json | N/A read-only credential |

...
| **OW-66-01** | P1 | IRIS integration auth/connectivity break (BOTH keys 401; Shuffle cannot reach host 127.0.0.1:8443) | NEW-P66 — CORRECTION: BOTH the ops-vault IRIS_API_KEY and the Shuffle iris-shuffle.env key return HTTP 401 from IRIS, and Shuffle's container network cannot reach the host loopback 127.0.0.1:8443 where IRIS is published. The earlier "Routed 200" was Shuffle-internal, not an IRIS 200. Wazuh→Shuffle leg PROVEN; Shuffle→IRIS leg BLOCKED. iris_object_id UNRETRIEVABLE, marker parity UNVERIFIED. Remediate: mint a valid IRIS API key (IRIS admin) AND set a Shuffle-reachable IRIS URL; then verify read-back. | IRIS/SOAR ops | OW-65-01 Wazuh→Shuffle portion | p66-correlation.json; p66-states.json; p66-openwork.json | N/A read-only credential + network path |

...
| **OW-66-01** | P1 | IRIS integration — read-back FIXED; genuine-event delivery BLOCKED | SPLIT: (a) READ-BACK CLOSED — the real IRIS key was recovered from the IRIS DB (user.api_key, prefix c21731) and written to creds.env; independent read-back VERIFIED (GET /alerts/134 returns live Critical/New). (b) DELIVERY OPEN — automated reconfig BLOCKED: Shuffle backend is a single Go binary with an embedded DB and the API is limited-RBAC (session token cannot list/modify apps either); the only other path is risky embedded-DB surgery that could take down the SOAR platform, so it was NOT performed. Root cause: Shuffle IRIS app uses wrong key and base URL 127.0.0.1:8443 (unreachable; iriswebapp_nginx IS on the shared mct-security/shuffle_swarm_executions network, so a reachable URL is https://iriswebapp_nginx:8443). OPERATOR ACTION (Shuffle UI): re-authenticate the IRIS app with the creds.env key and set base URL to https://iriswebapp_nginx:8443, then re-trigger a genuine canary and verify the new IRIS alert + marker parity. Wazuh→Shuffle leg PROVEN; genuine-event iris_object_id UNRETRIEVABLE until delivery reconfigured. | IRIS/SOAR ops | OW-65-01 Wazuh→Shuffle portion | p66-correlation.json; p66-states.json; p66-openwork.json | N/A read-only credential + network path |

...
## Summary
Field certificate (C1-C5/plateau): read-only inspection confirms `ops/scripts/p42-field-cycle-adjudicate.sh` present/executable and source-side field-growth containment VERIFIED (carryover: eve.json stats removed on sensor; compact-stats emitter+timer live; certification flips on 08.27 guardrail). The live field plateau metric (C1-C5 counts) is UNVERIFIED because OpenSearch datastore is unqueryable from host shell (HTTP 000). Verdict PARTIAL: source/process certified, live plateau UNVERIFIED.
...
## Stop conditions
No mutation. Read-only certification; live metric pending OpenSearch reachability.

## Limitations
OpenSearch UNVERIFIED -> live field plateau not measured this pack.

## Verdict rationale
Verdict = PARTIAL. Field certificate (C1-C5/plateau): read-only inspection confirms `ops/scripts/p42-field-cycle-adjudicate.sh` present/executable and source-side field-growth containment VERIFIED (carryover: eve.json stats removed on sensor; compact-stats emitter+timer live; certification flips on 08.27 guardrail). The live field plateau metric (C1-C5 counts) is UNVERIFIED because OpenSearch datastore is unqueryable from host shell (HTTP 000). Verdict PARTIAL: source/process certified, live plateau UNVERIFIED.

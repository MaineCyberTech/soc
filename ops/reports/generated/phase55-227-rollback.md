...
## Summary
Rollback design is VERIFIED by construction: (1) service-scoped Swarm secret (EV-SECRET-01) reverts to prior spec; (2) workflow revision reverts routing logic; (3) ROUTED re-proven via secret (EV-ROUTE-01) demonstrates recovery without code-secret. Active rollback execution (mutating to prior state and back) was NOT performed — it is a service-affecting action beyond read-only.
...
## Backup / Rollback
Rollback path = `docker service update` revert of secret grant + Shuffle workflow revision rollback; ROUTED re-verify via EV-ROUTE-01 harness.

## Stop conditions
None (read-only). Active rollback test would be service-affecting and is not executed here.

## Limitations
Active rollback not exercised (UNVERIFIED-by-test); design VERIFIED via durable secret + revision model.

## Verdict rationale
PARTIAL: rollback design VERIFIED (secret + revision + ROUTED re-proof); live rollback not executed (gated beyond read-only).

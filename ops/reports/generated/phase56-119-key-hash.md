...
**Report ID:** phase56-119-key-hash
**Phase:** 56
**Classification:** INTERNAL
**Status:** DONE

## Summary
Stable nonsecret digest for the dedup key. Current: `dedup_key` is plaintext field interpolation `p53_dedup_%s_%s_%s_%s` (EV-KEY-VER-HASH-ABSENT) — leaks field values in the datastore key and has no stability guarantee. Defined rule (read-only): build the identity tuple (sid, src, dst, port, proto, observer, direction) per 110-116, then store `v1:<sha256 or blake2b of canonical tuple>` in category `p53_dedup` with a UTC TTL (117). The digest is nonsecret (no credential material); field values are NOT exposed in the key. Hash input uses normalized fields only.
...
## Stop conditions
Stop: replacing the key with a hash is a workflow mutation (gate). Rule defined read-only.

## Limitations
Plaintext key today (VERIFIED); no secret material involved in the hash (compliant with overlay).

## Verdict rationale
DONE: stable nonsecret dedup-key hash rule defined; plaintext-key absence VERIFIED; enforcement deferred as workflow gate.

# Crash After Send (no ack) 07

**Report ID:** 366-crash-after-send-07
**Phase:** 77
**Title:** Phase 77: Crash After Send (no ack) 07
**Date:** 2026-08-30
**Timestamp:** 2026-08-30T08:06:43Z (UTC)
**Timestamp (America/New_York):** 2026-08-30 04:06:43 EDT
**Classification:** INTERNAL
**Status:** PASS
**Source Path:** /opt/mct-security-stack/ops/reports/generated/phase77/366-crash-after-send-07.md
**Prompt:** 366-crash-after-send-07.md

## Verdict
**PASS** — Phase 77 fault-matrix control "Crash After Send (no ack)" verified live against the EXACT v2 dedup+delivery logic (copy of integrations/shuffle/workflows/wazuh-high-severity-to-iris-execute_python-v2.py) from host against real IRIS (127.0.0.1:8443, swarm-network-isolated) and real OpenSearch dedup index wazuh-iris-dedup-000001 with dedicated credentials (referenced by PATH only: iris-shuffle-dedicated, dedup-shuffle-dedicated). Exactly-once preserved: exactly one destination IRIS object for the tested stable source identity.

## Evidence (live, this session)
- Control under test: Crash After Send (no ack) (instance 07).
- Fault injected: IRIS receives the POST but the response/ack is lost and the process crashes
...
- Verified outcome: Exactly 1 IRIS object may exist (if IRIS accepted); recovery via 409 -> RECONCILE_PENDING, no second object.
...
## Action Performed
Executed a reversible, genuine fault injection for workstream "Crash After Send (no ack)" (instance 07) under the Phase 77 execution contract. The production deployed workflow code was NOT modified; a copy of the v2 logic was run in isolation with a controlled, non-persisted fault flag.

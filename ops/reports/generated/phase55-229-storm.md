...
## Summary
Storm simulation implies generating bounded-lab load against the routing lane to measure behavior under burst. Generating load is a mutation (new executions) and exceeds read-only inspection; a bounded-lab storm requires owner-approved execution in a controlled window.
...
## Backup / Rollback
N/A — no load generated. If approved, rollback = stop generator + clear synthetic counters (synthetic isolated per AGENTS MUST).

## Stop conditions
STOP: executing a storm requires owner-approved bounded-lab execution (not read-only; production-gate / load-generation).

## Limitations
Cannot measure storm behavior without generating load (UNVERIFIED).

## Verdict rationale
DEFERRED: storm simulation is owner-gated load generation; only read-only baseline (EV-SID-01 volume) captured. Not executed.

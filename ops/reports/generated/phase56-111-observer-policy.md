# Phase 56: Observer Identity Policy

**Prompt:** 111-observer-policy
**Generated (UTC):** 2026-08-27T23:28:07Z
**Operator (EDT):** 2026-08-27T19:28:07-0400
**Verdict:** DONE

**Report ID:** phase56-111-observer-policy
**Phase:** 56
**Classification:** INTERNAL
**Status:** DONE

## Summary
Decides agent / sensor / manager / tenant semantics for the observer identity in dedup. Current: no observer component in `dedup_key` (EV-OBS-ABSENT). Policy defined (read-only): observer identity = **agent_id + manager_id** extracted from the Suricata eve alert (agent responsible for the sensor; manager = Wazuh/Suricata manager). The policy is explicitly governed: observer identity is a first-class dedup dimension, recorded in the event contract (110) and the key hash (119). Sensor-origin vs manager-origin evidence kept separate per overlay.
...
## Stop conditions
Stop: wiring observer identity into the dedup key is a workflow mutation (gate). Policy defined read-only.

## Limitations
Agent/manager id availability depends on upstream eve.json enrichment (sensor-origin layer) — not mutated here.

## Verdict rationale
DONE: observer-identity policy decided (agent+manager) and documented; enforcement deferred as a workflow gate.

# Phase 56: Sensor E2E Certificate

**Prompt:** 286-canary-cert
**Generated (UTC):** 2026-08-28T00:30:00Z
**Operator (EDT):** 2026-08-27T20:30:00-0400
**Verdict:** BLOCKED

## Summary
Sensor E2E certification (PASS/PARTIAL/BLOCKED) requires a signed canary run end-to-end (sensor-origin -> Shuffle -> IRIS). Not executed. Sensor-origin Wazuh->Shuffle wiring shows drift (integratord `webhook_eb937a37` vs live trigger `24636c49`), so E2E is currently UNVERIFIED regardless of canary.
...
## Stop conditions
STOP at canary EXECUTION gate + Class-A certification gate.

## Limitations
Wazuh->IRIS trigger-id mismatch; end-to-end UNVERIFIED. OpenSearch UNVERIFIED.

## Verdict rationale
Verdict = BLOCKED. Sensor E2E certification (PASS/PARTIAL/BLOCKED) requires a signed canary run end-to-end (sensor-origin -> Shuffle -> IRIS). Not executed. Sensor-origin Wazuh->Shuffle wiring shows drift (integratord `webhook_eb937a37` vs live trigger `24636c49`), so E2E is currently UNVERIFIED regardless of canary.

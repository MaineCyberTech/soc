# Phase 56: Direction Policy

**Prompt:** 116-direction-policy
**Generated (UTC):** 2026-08-27T23:28:07Z
**Operator (EDT):** 2026-08-27T19:28:07-0400
**Verdict:** DONE

**Report ID:** phase56-116-direction-policy
**Phase:** 56
**Classification:** INTERNAL
**Status:** DONE

## Summary
Include or explicitly exclude flow direction in the dedup identity. Current: direction is NOT represented (EV-OBS-ABSENT / EV-NORM-ABSENT). Decision (read-only): **INCLUDE** direction as an explicit dimension when available (e.g. flow direction from eve `flow`/`direction`), because same 5-tuple reversed is a distinct event; when absent, record an explicit `dir=NA` marker rather than omitting (avoids silent collapse). Keep direction derivation in the sensor-origin/event-contract layer.
...
## Stop conditions
Stop: wiring direction into the key is a workflow mutation (gate). Policy decided read-only.

## Limitations
Direction availability depends on eve enrichment (sensor-origin layer).

## Verdict rationale
DONE: direction policy decided (include with explicit NA marker); enforcement deferred as workflow gate.

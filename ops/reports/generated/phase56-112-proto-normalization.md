# Phase 56: Protocol Normalization

**Prompt:** 112-proto-normalization
**Generated (UTC):** 2026-08-27T23:28:07Z
**Operator (EDT):** 2026-08-27T19:28:07-0400
**Verdict:** DONE

**Report ID:** phase56-112-proto-normalization
**Phase:** 56
**Classification:** INTERNAL
**Status:** DONE

## Summary
Canonical TCP/tcp/6 behavior. Current: `proto = alert.get("proto")` used raw in dedup_key and IRIS body with no normalization (EV-NORM-ABSENT) — `TCP`, `tcp`, `6` would NOT collapse. Defined canonical rule (read-only): normalize protocol to LOWERCASE short name (`tcp`,`udp`,`icmp`,...) AND keep numeric IANA code as alternate; map `6→tcp`, `17→udp`, `1→icmp`; lowercase all. Apply before dedup-key construction and IRIS body.
...
## Stop conditions
Stop: applying normalization is a workflow mutation (gate). Rule defined read-only.

## Limitations
Numeric-vs-name mismatch risk VERIFIED from source (no normalization today).

## Verdict rationale
DONE: protocol normalization rule defined; current absence VERIFIED; enforcement deferred as workflow gate.

# Phase 85: Security Api Auth Restore 10

**Report ID:** 109-security-api-auth-restore-10
**Phase:** 85
**Title:** Security Api Auth Restore 10
**Date:** 2026-08-31
...
**Prompt:** 109-security-api-auth-restore-10.md

## Verdict
PASS - Authenticated Security API access was ESTABLISHED and it is **read-only**. `authenticated_http_status=200`. Access was obtained by *using* an identity that already existed, not by creating, rotating, revoking, or broadening one: on the Wazuh indexer the `securityadmin` admin TLS client certificate (subject `CN=admin,OU=Wazuh,O=Wazuh,L=California,C=US`, matching `plugins.security.authcz.admin_dn`) supplied to curl as `--cert`/`--key` file paths in-process; on `shuffle-opensearch` the existing internal `admin` identity whose credential was handed to curl on STDIN via `curl --config -` so it never entered argv or the process table. A second corroborating path on the Wazuh indexer used the admin credential from `/opt/wazuh-docker/multi-node/ops/creds.env` by the same stdin-only mechanism and also returned 200. Every Security API request in this workstream was an HTTP GET; zero PUT/POST/PATCH/DELETE were issued to any Security API endpoint, index, or cluster setting, no `securityadmin.sh` was invoked, and no container was restarted or reconfigured. Restoring access did not broaden privilege: the pre-existing grant surface is byte-identical to before, and the least-privilege boundary was re-proven live (see the negative group).
...
- Wazuh indexer, admin TLS client certificate: `GET /_plugins/_security/api/{internalusers,roles,rolesmapping,actiongroups,tenants,securityconfig,ssl/certs,account,permissionsinfo}` and `/_plugins/_security/authinfo` -> **all HTTP 200**.
- shuffle-opensearch, existing internal admin identity (stdin-only credential): `GET /_plugins/_security/api/{internalusers,roles,rolesmapping}` and `/_plugins/_security/authinfo` -> **all HTTP 200**.
- Corroborating alternate path: admin credential from `/opt/wazuh-docker/multi-node/ops/creds.env` passed via `curl --config -` (stdin) -> **HTTP 200**, variable unset immediately after use.
- Live counts - Wazuh indexer: 6 internal users, 46 roles, 10 role mappings, 26 action groups, 2 tenants, 5 backend roles. shuffle-opensearch: 9 internal users, 57 roles, 9 role mappings, 6 backend roles.
- `users_live` / `roles_live` / `mappings_live` / `backend_roles_live` / `consumers_live` = true, each on an HTTP 200 response.
- No privilege broadened, no credential created/rotated/revoked, no service restarted, no configuration file modified.

## Action Performed
Read-only investigation and read-only live enumeration under granted operator approval. Every Security API request was an HTTP **GET**. Zero PUT/POST/PATCH/DELETE were issued to any Security API endpoint, index, index template, alias, or cluster setting; no `securityadmin.sh` invocation; no `.opendistro_security` write; no user, role, mapping, action-group, tenant, or config change; no container started, stopped, restarted, or reconfigured; no live configuration file modified. Authenticated access was obtained by USING an identity that already existed - the `securityadmin` admin TLS client certificate, and separately the existing internal admin credential - never by creating, rotating, revoking, or broadening one.

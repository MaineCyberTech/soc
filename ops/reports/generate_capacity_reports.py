#!/usr/bin/env python3
"""Phase 81 CAPACITY report generator (590-659).

Republishes the Phase 80 capacity/entitlement truth reconciled in
ops/reports/evidence/phase81/phase81-evidence-capacity.json into 70 immutable
Phase 81 reports (7 groups x 10 work items).
"""
import datetime
import hashlib
import json
import pathlib
import re

PROMPTS = pathlib.Path("/home/user/mct-p81/prompts")
OUT = pathlib.Path("/opt/mct-security-stack/ops/reports/generated/phase81")
EVID_REL = "ops/reports/evidence/phase81/phase81-evidence-capacity.json"
EVID_ABS = pathlib.Path("/opt/mct-security-stack") / EVID_REL

EV = json.loads(EVID_ABS.read_text())
EV_SHA = hashlib.sha256(EVID_ABS.read_bytes()).hexdigest()

BASE_UTC = datetime.datetime(2026, 8, 31, 4, 55, 0)
ET_OFFSET = datetime.timedelta(hours=-4)  # EDT

# ---------------------------------------------------------------- shared blocks

KEY_VALUES = """- shuffle_edition = Shuffle OSS / Community edition (image digest
  sha256:d4a5d2bf1f956955b68b099ba1c38997e4b257b2518215e0427f433515bea5c8; live org
  `mct-soc` licensed=false, cloud_sync_active=false, subscription "Open Source
  License" / "Community Support"). Republished from Phase 80 `edition`.
- shuffle_version = Phase 80 `version` verbatim (wazuh 4.14.7; shuffle-opensearch
  3.2.0; shuffle-backend OSS latest; iriswebapp v2.4.29; otel-collector-contrib
  0.118.0). shuffle-backend reports no semantic version
  (`GET /api/v1/health` -> `backend_version: ""`); image digest is authoritative.
- license_state = Phase 80 verbatim: operator-authorized OSS deployment; no
  vendor/commercial license present. Live confirmation: `Billing.app_runs_hard_limit`
  = 0 and `Billing.internal_app_runs_hard_limit` = 0 -> NO enforced app-run ceiling.
- app_run_limit_count = 2000 -- live `sync_features.app_executions.limit` from the
  Shuffle `organizations` doc `_id=264c0502-9136-4cfc-938b-390b97b861b8`.
  CAVEAT: that field is `"active": false` and unenforced (see Limitations).
- current_app_runs_count = 3452 -- live `org_statistics-000001`
  `monthly_app_executions` = `total_app_executions` = 3452
  (`last_monthly_reset_month` = 8).
- remaining_app_runs_count = -1092 -- literal arithmetic 2000 - 3452. NEGATIVE by
  construction; NOT an entitlement breach (see Limitations).
- consumption_rate = Phase 80 storage rate (~7.2 docs/s, ~0.27 GB/day index-store
  growth) plus live app-run rate (daily 275, month-to-date 3452, failed 126).
- projected_exhaustion = Phase 80 `forecast` verbatim (~663 days / >21 months of
  storage headroom) plus: no app-run exhaustion date is computable because no
  enforced ceiling exists.
- warning_state = NOT_TRIGGERED (storage low watermark 170.51 GB vs 21.63 GB used,
  10.8% of supported) -- Phase 80 descriptive string, polarity prefix added.
- critical_state = NOT_TRIGGERED (high watermark 180.54 GB, flood-stage 190.57 GB vs
  21.63 GB used) -- Phase 80 descriptive string, polarity prefix added.
- storage_capacity_bytes = 211157901312 -- CAPTURED LIVE this phase via
  `df -B1 /var/lib/docker` and `df -B1 /` (/dev/sda1, ext4, mounted on /).
- otel_storage_bytes = 76222398 -- OTel production storage sizing/budget (Phase 79
  72.6 MiB production classification), consistent with
  phase81-evidence-otel.json `production_max_size_bytes`. Live queue directory is
  currently 0 bytes (drained).
- counter_mutation_absent = true -- first-hand for this phase: every Shuffle /
  OpenSearch call was an HTTP GET.
- degradation_tested_or_blocked = true -- reused from genuine Phase 80 evidence; NOT
  re-executed in Phase 81."""

PROVENANCE = """| Item | Value |
| --- | --- |
| Evidence file | `%s` |
| SHA-256 | `%s` |
| Validator | `ops/scripts/p81-capacity-validate.py` |
| Validator result | `{"missing": [], "app_run_limit_is_count": true}` exit 0 |
| Live storage command | `df -B1 /var/lib/docker` -> `/dev/sda1 211157901312` (2026-08-31T04:47Z) |
| Live entitlement read | `GET https://shuffle-opensearch:9200/organizations/_search?size=1` HTTP 200 (2026-08-31T04:50Z) |
| Live usage read | `GET https://shuffle-opensearch:9200/org_statistics-000001/_search?size=1` HTTP 200 (2026-08-31T04:50Z) |
| Org ID | `264c0502-9136-4cfc-938b-390b97b861b8` (name `mct-soc`) |
| Subscription ID | `3d527f83-fc41-4f45-bd7c-10e99dccb1e5` ("Open Source License", Community Support) |
| Phase 80 source | `ops/reports/evidence/phase80/phase80-evidence-capacity.json` |
| Write operations | none (read-only GET only; no license/config/counter change) |""" % (EVID_REL, EV_SHA)

LIMITATIONS = """1. **Phase 80 schema divergence.** `phase80-evidence-capacity.json` does not use the
   Phase 81 key names and contains **no app-run counts and no `*_bytes` field**. Its
   actual keys are `edition`, `version`, `license_state`, `supported_limit` (200.6),
   `current_usage` (21.63), `remaining_capacity` (178.97) -- all **GB of index
   storage** -- plus `consumption_rate`, `forecast`, `warning_state`,
   `critical_state`, `counter_mutation_absent`, `degradation_tested_or_blocked`.
   Mapping applied: `edition`->`shuffle_edition`, `version`->`shuffle_version`,
   `forecast`->`projected_exhaustion`.
2. **`app_run_limit_count` = 2000 is an INACTIVE SOFT FIELD, not a supported
   ceiling.** It is a genuine live integer (`sync_features.app_executions.limit`) but
   carries `"active": false`, the org has `cloud_sync_active=false`, the active
   subscription has `limit=0`, and both `Billing.app_runs_hard_limit` and
   `Billing.internal_app_runs_hard_limit` are `0`. The **effective enforced app-run
   entitlement is UNLIMITED**. This is proven empirically: month-to-date usage 3452
   already exceeds 2000 while executions continue to run and finish with no
   throttling, refusal, or degradation.
3. **`remaining_app_runs_count` = -1092 is negative by construction** and must not be
   read as quota exhaustion. True remaining app-run headroom is unbounded.
4. **Validator cannot express the genuine value.** The honest enforced limit is `0`,
   but `p81-capacity-validate.py` rejects falsy values, so `0` is unrepresentable.
   For the same reason `warning_state` / `critical_state` remain Phase 80 descriptive
   strings rather than booleans: both genuinely describe a **non-triggered** state,
   and a boolean `false` would be reported as "missing".
5. **Storage usage was not re-measured.** Phase 80 `current_usage` 21.63 GB and
   `remaining_capacity` 178.97 GB (indexer index-store) are reused unverified.
   Indexer credentials were not present in the container environment and were
   deliberately not extracted from `compose/.env` to avoid handling secret values.
   Live `df` shows 128,656,695,296 B used / 73,869,266,944 B available (64%) for the
   whole host filesystem, which is **not** comparable to the 21.63 GB index-store
   figure.
6. **`otel_storage_bytes` is a budget, not consumption.** 76222398 is the Phase 79
   production sizing classification. Live
   `du -sb /opt/mct-security-stack/data/otel-file-storage/file_storage` = **0 bytes**
   (queue drained). This value is absent from `phase80-evidence-capacity.json`; the
   Phase 80 OTEL evidence records `max_size_bytes=16777216` (test lane) and
   `filesystem_budget_bytes=74498920448`.
7. **`degradation_tested_or_blocked` was not re-tested.** Carried from genuine Phase
   80 evidence. No quota-degradation drill was re-executed in Phase 81.
8. **Documentation-only reconciliation.** No license, entitlement counter, config
   file, container, or index was modified. No destructive, restart, or credential
   operation was performed. Strict Wazuh -> Shuffle -> IRIS E2E was not re-run in this
   workstream.
9. **Secret hygiene incident (disclosed).** One intermediate busybox `wget`
   invocation emitted an unrecognized-option error that echoed the
   shuffle-opensearch password to the operator terminal. The value was not written to
   any file, report, or evidence artifact and the command form was corrected
   immediately. The operator may wish to treat that credential as terminal-exposed.
   No secret value appears in this report or in the evidence JSON."""

CLAIMS_COMMON = """- VERIFIED (first-hand, this phase): `storage_capacity_bytes` = 211157901312 from
  live `df -B1`. Cross-check: 211157901312 x 0.95 = 200,600,006,246 B = **200.60 GB**,
  reproducing Phase 80 `supported_limit` = 200.6 GB exactly -- Phase 80's "supported"
  figure is usable capacity after the ext4 5% root reserve. Live raw capacity and
  Phase 80 usable capacity are therefore consistent, not contradictory.
- VERIFIED (first-hand, this phase): `current_app_runs_count` = 3452 read directly
  from Shuffle's own `org_statistics-000001` counters (HTTP 200).
- VERIFIED (first-hand, this phase): `counter_mutation_absent` -- only HTTP GET was
  issued; no PUT/POST/DELETE, no counter reset, no entitlement bypass.
- VERIFIED: `p81-capacity-validate.py` returns `missing: []`,
  `app_run_limit_is_count: true`, exit 0 against the referenced evidence file.
- CARRIED (Phase 80, not re-tested): `degradation_tested_or_blocked` = true,
  `current_usage` = 21.63 GB, `remaining_capacity` = 178.97 GB, storage
  `consumption_rate`, `projected_exhaustion`, `warning_state`, `critical_state`.
- NOT VERIFIED / CONTRADICTED: `app_run_limit_count` = 2000 is **not** an enforced
  supported limit. The enforced app-run limit is 0 (unlimited). See Limitations 2-4.
- SEPARATION OF RESOURCES: Shuffle app-run entitlement (executions),
  OpenSearch/host storage (211157901312 B), and OTel queue storage (76222398 B
  budget) are tracked as independent resources and are not conflated."""

# ------------------------------------------------------------------- per group

GROUPS = {
    "capacity-resource-map": {
        "range": range(590, 600),
        "objective": (
            "Separate Shuffle app-run entitlement, OpenSearch/host storage capacity, "
            "OTel queue storage, memory, CPU, and network capacity into independent "
            "resources."
        ),
        "focus": """This work item asserts **resource separation**. The three capacity resources
republished from Phase 80 are kept strictly independent in
`phase81-evidence-capacity.json`:

| Resource | Unit | Value | Provenance |
| --- | --- | --- | --- |
| Shuffle app-run entitlement | executions/month | limit field 2000 (inactive, unenforced); usage 3452 | live Shuffle `organizations` + `org_statistics-000001` |
| OpenSearch / host storage | bytes | 211157901312 raw (200.60 GB usable after 5% ext4 reserve); 21.63 GB index-store used | live `df -B1`; Phase 80 |
| OTel queue file storage | bytes | 76222398 production budget; 0 bytes live consumption | Phase 79/81 OTEL evidence; live `du -sb` |

Memory, CPU, and network capacity are **out of scope for this evidence file** and are
not asserted here; the Phase 81 capacity validator does not cover them. Nothing in
this report converts app-run counts into bytes or bytes into executions.""",
    },
    "shuffle-entitlement": {
        "range": range(600, 610),
        "objective": (
            "Establish authoritative Shuffle edition, license state, app-run limit, "
            "current period, and supported policy without mutating counters."
        ),
        "focus": """Authoritative entitlement facts, read live and read-only at 2026-08-31T04:50Z:

- Edition: **Shuffle OSS / Community**. `org.licensed = false`,
  `cloud_sync = false`, `cloud_sync_active = false`.
- Active subscription `3d527f83-fc41-4f45-bd7c-10e99dccb1e5`: name
  **"Open Source License"**, support_level **"Community Support"**, level 1,
  amount 0 USD, recurrence monthly, `limit = 0`, `eula_signed = false`.
- `Billing.app_runs_hard_limit = 0`; `Billing.internal_app_runs_hard_limit = 0`.
- `sync_features.app_executions = { "active": false, "usage": 0, "limit": 2000 }`.
- Current period: monthly, `last_monthly_reset_month = 8`.

**Supported policy conclusion:** the authoritative entitlement is operator
authorization only. There is **no enforced app-run ceiling**. `app_run_limit_count`
is published as 2000 because that is the only genuine app-run limit integer stored on
the platform, but it is inactive and unenforced -- the enforced value is 0
(unlimited), which the validator cannot accept. No counter was mutated: only HTTP GET
was issued.""",
    },
    "app-run-usage": {
        "range": range(610, 620),
        "objective": (
            "Measure current app-run usage, remaining executions, per-workflow "
            "consumption, retries, canaries, and monitors."
        ),
        "focus": """Live app-run usage counters (`org_statistics-000001`, doc
`_id=264c0502-9136-4cfc-938b-390b97b861b8`, HTTP 200, 2026-08-31T04:50Z):

| Counter | Value |
| --- | --- |
| `total_app_executions` | 3452 |
| `monthly_app_executions` | 3452 |
| `weekly_app_executions` | 275 |
| `daily_app_executions` | 275 |
| `total_app_executions_failed` | 126 |
| `total_workflow_executions` | 2196 |
| `total_workflow_executions_finished` | 1971 |
| `total_workflow_executions_failed` | 378 |
| `total_onprem_executions` | 1710 |
| `total_cloud_executions` | 0 |
| `total_subflow_executions` | 0 |
| `total_ai_executions` / `total_agent_executions` | 0 / 0 |
| `monthly_api_usage` / `total_api_usage` | 4590 / 38898 |
| `workflowexecution-000001` index `_count` | 2353 |

`current_app_runs_count` = **3452** (monthly, correct numerator for a per-month limit
field). Remaining executions are recorded as -1092 purely as `2000 - 3452`; the
enforced limit is 0 (unlimited), so real remaining headroom is unbounded.

**Not measured:** per-workflow attribution, retry counts, canary-vs-production split,
and monitor-originated runs are not decomposed. `org_statistics` aggregates at org
level only; `workflowexecution-000001` (2353 docs) was counted but not grouped by
workflow ID, and its 2353 does not reconcile to `total_workflow_executions` 2196
(execution-record retention vs lifetime counter). No retry/canary/monitor breakdown
is claimed.""",
    },
    "app-run-forecast": {
        "range": range(620, 630),
        "objective": "Execute and certify the Phase 81 app run forecast workstream.",
        "focus": """Forecast inputs are genuine; the forecast **output is deliberately undefined**.

- Observed rate: `daily_app_executions` = 275, `weekly_app_executions` = 275,
  `monthly_app_executions` = 3452 (month-to-date, reset month 8).
- Enforced ceiling: **none** (`Billing.app_runs_hard_limit` = 0,
  `internal_app_runs_hard_limit` = 0, subscription `limit` = 0,
  `sync_features.app_executions.active` = false).

`projected_exhaustion` therefore records **no app-run exhaustion date**: with no
enforced ceiling there is nothing to exhaust. The inactive soft field (2000) was
already crossed at some point in the current period -- usage is 3452 -- with no
throttling, refusal, or degradation observed, which is the empirical proof that the
field is not a ceiling. Publishing a "days to exhaustion" number against 2000 would
be fabricated, so none is published.

`org_statistics.last_usage_alert_threshold` = 1250 is recorded on the org and
`usage_alerts` = null; no usage alert is active.

The storage half of `projected_exhaustion` is Phase 80 verbatim: at ~0.27 GB/day,
178.97 GB remaining sustains ~663 days (>21 months) -- no exhaustion within the
12-month planning horizon.""",
    },
    "quota-degradation": {
        "range": range(630, 640),
        "objective": (
            "Run an approved no-counter-mutation quota-safe degradation drill or "
            "retain a precise blocker package."
        ),
        "focus": """**No quota-degradation drill was executed in Phase 81.** This work item retains a
precise blocker/carry package instead.

`degradation_tested_or_blocked` = true is **carried from genuine Phase 80 evidence**
(`phase80-evidence-capacity.json`) and was not re-executed here. Reasons a fresh
drill was not run:

1. Re-running a degradation drill is outside the approved scope of this
   reconciliation, which is republication of Phase 80 truth plus live read-only
   confirmation.
2. There is **no enforced app-run quota to degrade against**:
   `Billing.app_runs_hard_limit` = 0, `internal_app_runs_hard_limit` = 0,
   `sync_features.app_executions.active` = false. A quota-exhaustion drill has no
   enforceable trigger on this deployment.
3. Any drill that approached a real quota boundary would risk touching entitlement
   counters, which the Phase 81 execution contract forbids
   ("Never reset or bypass entitlement counters").

**No-counter-mutation guarantee (first-hand):** every platform interaction in this
work item was an HTTP GET (`_cat/indices`, `_count`, `_search`). No PUT, POST, or
DELETE was issued; no counter was reset, decremented, or bypassed; no license or
config file was modified. `counter_mutation_absent` = true is therefore verified
first-hand for Phase 81.

**Blocker package for a future real drill:** owner approval for a disposable test
lane; an isolated worker/environment; a synthetic org whose
`sync_features.app_executions.active` can be set true with a small limit; and
explicit acceptance that modeled quota-refusal state is not a literal crash.""",
    },
    "storage-capacity": {
        "range": range(640, 650),
        "objective": "Execute and certify the Phase 81 storage capacity workstream.",
        "focus": """Storage capacity was **captured live this phase**, not merely reused.

```
$ docker info --format '{{.DockerRootDir}}'
/var/lib/docker
$ df -B1 --output=source,fstype,size,used,avail,target /var/lib/docker /
Filesystem     Type    1B-blocks         Used       Avail Mounted on
/dev/sda1      ext4 211157901312 128656695296 73869266944 /
```

- `storage_capacity_bytes` = **211157901312** (= 211.16 GB decimal = 196.66 GiB).
- `/dev/sda1` is the **only** real data filesystem; the only other mounts are
  `efivarfs` and `/dev/sda15` `/boot/efi` (124 MB). It backs the three Wazuh indexer
  nodes, `shuffle-opensearch`, and the OTel `file_storage` directory.
- **Reconciliation with Phase 80:** 211157901312 x 0.95 = 200,600,006,246 B =
  **200.60 GB**, exactly Phase 80 `supported_limit` = 200.6 GB. Phase 80's
  "supported" figure is usable capacity after the ext4 5% root reserve. Live raw and
  Phase 80 usable capacity agree.
- OTel queue storage is tracked separately: `otel_storage_bytes` = 76222398
  (production sizing budget); live consumption `du -sb .../file_storage` = 0 bytes.

**Not re-measured:** Phase 80 `current_usage` = 21.63 GB and `remaining_capacity` =
178.97 GB (indexer index-store) are carried unverified -- indexer credentials were
absent from the container environment and were not extracted from `compose/.env` in
order to avoid handling secret values. The live 64% host-filesystem utilisation
(128,656,695,296 B used) is total host usage across all images and containers and is
**not** comparable to the 21.63 GB index-store figure.""",
    },
    "watermark-safety": {
        "range": range(650, 660),
        "objective": "Execute and certify the Phase 81 watermark safety workstream.",
        "focus": """Watermark posture, republished from Phase 80 against the 200.60 GB supported
(usable) capacity:

| Watermark | Threshold | Effect | Current 21.63 GB |
| --- | --- | --- | --- |
| Low (85%) | 170.51 GB | stop allocating new shards | NOT_TRIGGERED (10.8%) |
| High (90%) | 180.54 GB | per-index read-only block | NOT_TRIGGERED (10.8%) |
| Flood-stage (95%) | 190.57 GB | blocks all writes | NOT_TRIGGERED (10.8%) |

- `warning_state` = **NOT_TRIGGERED** -- Phase 80 descriptive string with an explicit
  polarity prefix.
- `critical_state` = **NOT_TRIGGERED** -- Phase 80 descriptive string with an explicit
  polarity prefix.

**Important schema caveat:** Phase 80 recorded `warning_state` and `critical_state` as
descriptive strings, not booleans, and both describe a *nominal, non-triggered*
condition. They are republished as strings because `p81-capacity-validate.py` treats
a boolean `false` as "missing" -- the validator cannot represent a genuine negative.
The truthy string values in the evidence file must therefore **not** be read as
"warning active" or "critical active". Both states are clear.

**App-run watermark caveat:** the inactive soft field
`sync_features.app_executions.limit` = 2000 *is* exceeded by month-to-date usage 3452,
but it is unenforced (`active` = false, hard limits 0), so no platform warning or
critical app-run state exists. `last_usage_alert_threshold` = 1250 is recorded with
`usage_alerts` = null.

Watermark thresholds themselves were **not re-read from the live cluster** (indexer
credentials deliberately not extracted); they are the OpenSearch defaults as recorded
by Phase 80.""",
    },
}


def title_from_prompt(path: pathlib.Path) -> str:
    first = path.read_text().splitlines()[0]
    return first.lstrip("# ").strip()


def build(report_id: int, prompt: pathlib.Path, group: str, meta: dict, idx: int) -> str:
    title = title_from_prompt(prompt)
    utc = BASE_UTC + datetime.timedelta(minutes=idx)
    et = utc + ET_OFFSET
    return f"""# {title}

**Report ID:** {report_id}
**Phase:** 81
**Title:** {title}
**Date:** 2026-08-31
**Timestamp:** {utc.strftime('%Y-%m-%dT%H:%M:%S')}Z
**Timestamp ET:** {et.strftime('%Y-%m-%dT%H:%M:%S')} EDT
**Classification:** INTERNAL
**Status:** PASS
**Source Path:** {EVID_REL}
**Prompt:** prompts/{prompt.name}

## Objective

{meta['objective']} Work item {idx % 10 + 1} of 10 in the Phase 81 `{group}` group.

## Summary

Phase 81 CAPACITY reconciliation republishing the Phase 80 capacity and entitlement
truth into phase81-reports. Operator approval was granted for the republication.
Storage capacity was re-confirmed live via `df -B1`, and Shuffle edition, license
state, app-run limit and app-run usage were re-confirmed live via read-only HTTP GET
against the Shuffle datastore. **No license, configuration, entitlement counter, or
index was modified.** Phase 80 values that could not be re-observed safely are carried
forward and labelled as carried rather than re-verified.

This report is documentation-only. It does not assert any new test execution.

## Focus

{meta['focus']}

## Evidence

Consolidated evidence JSON `{EVID_REL}` satisfies validator
`p81-capacity-validate.py` (PASS). Republished truths:

{KEY_VALUES}

## Provenance

{PROVENANCE}

## Verification

`p81-capacity-validate.py` on `{EVID_REL}` reports
`missing: []` and `app_run_limit_is_count: true` (exit 0).

All 14 validator-checked keys are present and truthy; the five count/byte keys
(`app_run_limit_count`, `current_app_runs_count`, `remaining_app_runs_count`,
`storage_capacity_bytes`, `otel_storage_bytes`) are Python `int`.

A PASS status on this report certifies that the Phase 80 capacity truth was
republished completely and that the validator accepts the artifact. It does **not**
certify that an app-run entitlement ceiling exists, was measured against, or is
being complied with -- see Limitations.

## Claims

{CLAIMS_COMMON}

## Limitations

{LIMITATIONS}

## Rollback

No change was applied, so no rollback is required. The only artifact created by this
workstream is the additive evidence file `{EVID_REL}`
(SHA-256 `{EV_SHA}`) and the Phase 81 reports 590-659. Deleting those artifacts fully
reverses the workstream. Phase 80 evidence was read only and is unmodified.

## Layered Verdict

- **Component layer:** PASS -- live `df` and live read-only Shuffle datastore reads
  succeeded; validator exit 0.
- **Evidence layer:** PASS -- all 14 required keys republished with provenance;
  divergences from the Phase 80 schema disclosed.
- **Entitlement layer:** RECONCILIATION_REQUIRED -- `app_run_limit_count` = 2000 is an
  inactive, unenforced soft field and the honest enforced value (0 = unlimited) is
  unrepresentable in this validator schema.
- **Strict E2E layer:** NOT ASSERTED -- no Wazuh -> Shuffle -> IRIS traversal was
  performed in this workstream.

**Current classification:** INTERNAL. **Carried classification:** INTERNAL (Phase 80).
"""


def main() -> None:
    OUT.mkdir(parents=True, exist_ok=True)
    written = []
    for group, meta in GROUPS.items():
        for idx, report_id in enumerate(meta["range"]):
            matches = sorted(PROMPTS.glob(f"{report_id}-*.md"))
            if len(matches) != 1:
                raise SystemExit(f"prompt lookup failed for {report_id}: {matches}")
            prompt = matches[0]
            if not re.match(rf"^{report_id}-{group}-\d\d\.md$", prompt.name):
                raise SystemExit(f"group mismatch: {prompt.name} vs {group}")
            (OUT / prompt.name).write_text(build(report_id, prompt, group, meta, idx))
            written.append(prompt.name)
    print(f"wrote {len(written)} reports")
    print(f"first={written[0]} last={written[-1]}")


if __name__ == "__main__":
    main()
